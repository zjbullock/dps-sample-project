...
public abstract class CrossActionTileBehaviorSO : BattleTileTargetBehaviorSO
{
    ...
    public override GenericDictionary<Vector3, CombatTileController> GetSelectActionTilesByAbilityRange(CombatTileController combatTile, int abilityRange, float abilityVerticalRange, float entityHeight,  bool isFlying)
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            foreach (RaycastHit raycast in raycastHits) {
                ...
                CombatTileController possibleTile = tileObject.GetComponent<CombatTileController>();
                if (possibleTile == combatTile) {
                    continue;
                }
                ...
                if (!combatTileRayCounter.ContainsKey(possibleTile)) {
                    combatTileRayCounter[possibleTile] = 0;
                } else {
                    combatTileRayCounter[possibleTile]++;
                }

                if (combatTileRayCounter[possibleTile] >= this.rayCastNecessary) {
                    bool added = this.AddPossibleTile(possibleActionTiles, new KeyValuePair<Vector3, CombatTileController>(possibleTile.Position, possibleTile));
                }
            }
        }
            ...
            foreach (RaycastHit raycast in raycastHits) {
                ...
                if (tileObject != null) {
                    ...
                    if (combatTileController != null) {
                        ...
                        // if (combatTileRayCounter[combatTileController] >= this.rayCastNecessary && ( !requireHeight || (requireHeight && (isFlying || (distance <= abilityVerticalRange)))) ) {
                        this.AddPossibleTile(possibleActionTiles, new KeyValuePair<Vector3, CombatTileController>(combatTileController.Position, combatTileController));
                    }
                }
            }
        }
